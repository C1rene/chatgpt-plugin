...
import { DateTimeTool } from '../utils/tools/DateTimeTool.js'
    ...
    if (use === 'bing') {
      ...
      while (keys.length >= 0) {
        ...
        try {
          ...
          err.response && logger.error(`claude api error response: ${JSON.stringify(err.response.data)}`)
      ...
      const image = await getImg(e)
      //logger.warn('[GEMINI DEBUG] image 来源:', image) 
      let imageUrl = image ? image[0] : undefined
      //logger.warn('[GEMINI DEBUG] imageUrl:', imageUrl)
      //logger.warn('[GEMINI DEBUG] e.message:', e.message)
      // ===== 引用解析共用函数（群聊/私聊通用）=====
      ...
      async function GetNodeMessageText (resid) {
        const messageSegments = []
        try {
          const forwardDetail = await e.bot.getForwardMsg(resid)
          // 兼容多种适配器返回结构：
          // - ICQQ: 直接是数组
          // - NapCat / OneBot v11: { messages: [...] } 或 { data: { messages: [...] } }
          const msgs = Array.isArray(forwardDetail)
            ? forwardDetail
            : (forwardDetail?.messages || forwardDetail?.data?.messages || [])

          if (!msgs.length) {
            messageSegments.push(`<SystemInfo type="warning">ToolError:Empty forward message</SystemInfo>`)
            return messageSegments.join('')
          }

          for (const msg of msgs) {
            const authorQQ = msg.sender?.user_id || msg.user_id || 'unknown'
            const authorName = msg.sender?.card || msg.sender?.nickname || msg.nickname || 'UnknownUser'
            const segments = msg.message || msg.content || []
            // 发送时间（Unix 秒 → 可读格式），缺失则留空
            const ts = msg.time ?? msg.data?.time ?? msg.sender?.time
            const timeAttr = ts ? ` time="${formatDate(new Date(ts * 1000))}"` : ''

            if (!Array.isArray(segments) || segments.length === 0) {
              messageSegments.push(`<SystemInfo type="warning">ToolError:Unable to read information/No information available</SystemInfo>`)
              continue
            }

            for (const segment of segments) {
              const segType = segment.type
              const messageContent = segment.text ?? segment.data?.text ?? ''

              if (segType === 'text') {
                messageSegments.push(`<MessageSegment Author="${authorQQ}" AuthorName="${authorName}"${timeAttr} type="text">${escapeXml(messageContent)}</MessageSegment>`)
              }
              else if (segType === 'image') {
                // Note: 未来需要OCR支持否则Token成本太高了，或者干脆就不处理图片了
                messageSegments.push(`<MessageSegment Author="${authorQQ}" AuthorName="${authorName}"${timeAttr} type="image">[Image message, unable to display]</MessageSegment>`)
              }
              else if (segType === 'node' || segType === 'forward') {
                const subId = segment.id || segment.data?.id
                if (subId) {
                  messageSegments.push(`<Chat_History Author="${authorQQ}" AuthorName="${authorName}"${timeAttr}>${await GetNodeMessageText(subId)}</Chat_History>`)
                } else {
                  messageSegments.push(`<SystemInfo type="warning">ToolError:Nested forward missing id</SystemInfo>`)
                }
              }
              else {
                messageSegments.push(`<MessageSegment Author="${authorQQ}" AuthorName="${authorName}"${timeAttr} type="${escapeXml(String(segType ?? 'unknown'))}">${escapeXml(messageContent)}</MessageSegment>`)
              }
            }
          }
        }
        catch (err) {
          logger.warn('获取node消息内容失败，消息id:', resid, '错误信息:', err?.message, err?.stack)
          messageSegments.push(`<SystemInfo type="error">ToolError:Unable to read node</SystemInfo>`)
        }
        return messageSegments.join('')
      }
      // ============================================
      ...
      const hasRealImage = 
        // 当前消息里有 image 段
        (Array.isArray(e.message) && e.message.some(m => m.type === 'image')) ||
        // 或者引用消息里有 image 段
        (Array.isArray(e.source?.message) && e.source.message.some(m => m.type === 'image'))

      if (!hasRealImage) {
        // 没真实图片，不管 getImg 返回啥（头像 fallback）都跳过
        imageUrl = undefined
      }
      if (imageUrl) {
        ...
        try{
          prompt += `<Quoted_message Author="${e.message.find(m => m.type === 'reply').qq}" type="image"></Quoted_message>`
        }
        catch(err) {//丢弃
          }
        option.image = base64Image.toString('base64')
      }
      // 私聊：引用信息不在 e.message 的 reply 段，而在 e.source
      if (!hasRealImage && e.source && !e.message.find(m => m.type === 'reply')) {
        const src = e.source
        const authorQQ = src.sender?.user_id || src.user_id || 'unknown'
        const srcMsg = Array.isArray(src.message) ? src.message : []
        const nodeSeg = srcMsg.find(m => m.type === 'node' || m.type === 'forward')

        if (nodeSeg) {
          // 引用的是合并转发消息，复用 GetNodeMessageText
          const nodeId = nodeSeg.id || nodeSeg.data?.id
          prompt += `<Quoted_message Author="${authorQQ}" type="msg">`
          prompt += `<Chat_History>`
          prompt += await GetNodeMessageText(nodeId)
          prompt += `</Chat_History>`
          prompt += `</Quoted_message>`
        } else if (srcMsg.some(m => m.type === 'image')) {
          // 引用的是图片
          prompt += `<Quoted_message Author="${authorQQ}" type="image"></Quoted_message>`
          // 如需把图片喂给 Gemini，可在此 fetch src.message 里的 image url 转 base64 赋给 option.image
        } else {
          // 引用的是文本
          const quotedText = (srcMsg.filter(m => m.type === 'text')
            .map(m => m.text || m.data?.text).join('')) || src.raw_message || ''
          if (quotedText.trim()) {
            prompt += `<Quoted_message Author="${authorQQ}" type="msg"> ${escapeXml(quotedText.trim())}</Quoted_message>`
          }
        }
      }
      if (!hasRealImage && Array.isArray(e.message) && e.message.find(m => m.type === 'reply')) {
        const replyElem = e.message.find(m => m.type === 'reply')
        const rId = replyElem.message_id || replyElem.id
        //const normalizedId = String(rId).startsWith('-') ? String(rId) : '-' + rId
        const normalizedId = rId

        // 主动拉取引用消息内容
        try {
          const result = await e.bot.getMsg(normalizedId)
          let nodeMessage = Array.isArray(result?.message) 
            ? result.message.find(m => m.type === 'node') 
            : null
          let hasNode = !!nodeMessage  // 名字改一下，原来那个变量名语义是反的，容易踩坑
          //logger.warn('[GEMINI DEBUG] getMsg result:', JSON.stringify(result))
          // result 本身就是消息对象，没有 .data 包装
          if (result && !hasNode) {
            const quotedText = result.message
              ?.filter(m => m.type === 'text')
              ?.map(m => m.text || m.data?.text)
              ?.join('') || result.raw_message || ''


            if (quotedText?.trim()) {
              const safeText = escapeXml(quotedText.trim());
              //option.promptAddition = `Quoted message: ${quotedText.trim()}`
              prompt += `<Quoted_message Author="${replyElem.qq}" type="msg"> ${safeText}</Quoted_message>`
              //logger.warn('[GEMINI DEBUG] 引用内容:', quotedText.trim().slice(0, 100))
            }
          }
          else if (result && hasNode) {
            const nodeId = nodeMessage.id || nodeMessage.data?.id
            prompt += `<Quoted_message Author="${replyElem.qq}" type="msg">`
            prompt += `<Chat_History>`
            prompt += await GetNodeMessageText(nodeId)   // 传 resid，不是 message_id
            prompt += `</Chat_History>`
            prompt += `</Quoted_message>`
          }
          else {
            prompt += `<Quoted_message Author="${replyElem.qq}" type="msg"><SystemInfo type="error">ToolError:Unable to obtain sending information</SystemInfo></Quoted_message>`
          }
        } catch (err) {
          logger.warn('[GEMINI DEBUG] 处理引用消息失败:', err?.message, err?.stack)
          prompt += `<Quoted_message Author="${replyElem.qq}" type="msg"><SystemInfo type="error">ToolError:Unable to obtain sending information</SystemInfo></Quoted_message>`
        }

        // 用户附带的文字
        const pureText = e.message
          .filter(m => m.type === 'text')
          .map(m => m.text)
          .join('')
          .trim()
        if (pureText) {
          //option.promptAddition = (option.promptAddition ? option.promptAddition + '\n' : '') +
          //  `User's additional message: ${pureText}`
        }

        //logger.warn('[GEMINI DEBUG] final promptAddition:', option.promptAddition)
      }
      //logger.warn('[GEMINI DEBUG] opt.enableSmart:', opt.enableSmart) 
      if (opt.enableSmart) {
        const {
          funcMap
        } = await collectTools(e)
        let tools = Object.keys(funcMap).map(k => funcMap[k].tool)
        //logger.warn('[GEMINI DEBUG] 注入工具数:', tools.length)
