          ...
          if (result && !hasNode) {
            ...
            async function GetNodeMessageText(resid) {
              ...
              try {
                ...
                //logger.warn('[FORWARD]', typeof forwardDetail, JSON.stringify(forwardDetail)?.slice(0, 500))
                ...
                for (const msg of msgs) {
                  ...
                  for (const segment of segments) {
                    ...
                    if (segType === 'text') {
                      messageSegments.push(`<MessageSegment Author="${authorQQ}" AuthorName="${authorName}" type="text">${escapeXml(messageContent)}</MessageSegment>`)
                    }
                    else if (segType === 'image') {
                      // Note: 未来需要OCR支持否则Token成本太高了，或者干脆就不处理图片了
                      messageSegments.push(`<MessageSegment Author="${authorQQ}" AuthorName="${authorName}" type="image">[Image message, unable to display]</MessageSegment>`)
                    }
                    else if (segType === 'node' || segType === 'forward') {
                      const subId = segment.id || segment.data?.id
                      if (subId) {
                        messageSegments.push(`<Chat_History Author="${authorQQ}" AuthorName="${authorName}">${await GetNodeMessageText(subId)}</Chat_History>`)
                      } else {
                        messageSegments.push(`<SystemInfo type="warning">ToolError:Nested forward missing id</SystemInfo>`)
                      }
                    }
                    else {
                      messageSegments.push(`<MessageSegment Author="${authorQQ}" AuthorName="${authorName}" type="${escapeXml(String(segType ?? 'unknown'))}">${escapeXml(messageContent)}</MessageSegment>`)
                    }
                  }
                }
              }
