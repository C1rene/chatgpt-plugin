...
/**
 * 给触发消息贴 QQ 表情回应（兼容 napcat-adapter / OneBot v11 / icqq）
 * @param {object} e        Yunzai 消息事件
 * @param {string|number} faceId  QQ 表情 ID，常用：60=咖啡 76=赞 201=点赞 9=哭
 */
async function sendReaction (e, faceId) {
  try {
    if (!e?.message_id) return
    const bot = e.bot || (typeof Bot !== 'undefined' ? (Bot[e.self_id] || Bot) : null)
    if (!bot) return
    const id = String(faceId)
    // 1) OneBot v11（napcat-adapter / ws-plugin）
    if (typeof bot.sendApi === 'function') {
      return await bot.sendApi('set_msg_emoji_like', { message_id: e.message_id, emoji_id: id })
    }
    // 2) 部分适配器直接暴露同名方法
    if (typeof bot.setMsgEmojiLike === 'function') {
      return await bot.setMsgEmojiLike(e.message_id, id)
    }
    // 3) icqq 原生（保留兼容，万一以后切回去）
    if (e.isGroup && e.group?.setReaction) {
      return await e.group.setReaction(e.message_id, id, true)
    }
    if (!e.isGroup && e.friend?.setReaction) {
      return await e.friend.setReaction(e.message_id, id, true)
    }
  } catch (err) {
    logger?.warn?.('[chatgpt] 贴表情失败', err)
  }
}
...
export class chatgpt extends plugin {
  ...
  async abstractChat (e, prompt, use, forcePictureMode = false) {
    ...
    // 调用 LLM 前先贴个「」表情，告诉调用者已经收到了
    await sendReaction(e, 285)
