...
import { LoginErrorCode, segment } from 'oicq'
...
export class CustomGoogleGeminiClient extends GoogleGeminiClient {
   ...
   */
  async sendMessage (text, opt = {}, retryTime = 3, errorOut = true) {
    //logger.warn('[GEMINI DEBUG] tools count:', this.tools?.length ?? 0)
    //logger.warn('[GEMINI DEBUG] tools names:', this.tools?.map(t => t.name)?.join(', ') ?? '(empty)')
    //logger.warn('[GEMINI DEBUG] opt.image:', !!opt.image)
    //logger.warn('[GEMINI DEBUG] opt.toolMode:', opt.toolMode)
    ...
    if (result.status !== 200) {
      ...
      // 5xx 是服务端临时错误，可重试
      if (result.status >= 500 && result.status < 600 && retryTime > 0 && errorOut){
        const wait = (4 - retryTime) * 500 + Math.floor(Math.random() * 100) // 指数退避 + 抖动
        logger.warn(`[Gemini] HTTP ${result.status}，${wait}ms 后重试，剩余 ${retryTime - 1} 次: ${errorText}`)
        await new Promise(resolve => setTimeout(resolve, wait))
        return this.sendMessage(text, opt, retryTime - 1, errorOut)
      }

      // 429 限流也可重试（带更长等待）
      if (result.status === 429 && retryTime > 0 && errorOut) {
        const wait = (4 - retryTime) * 2000 + Math.floor(Math.random() * 1000) // 限流等待更久
        logger.warn(`[Gemini] HTTP 429 限流，${wait}ms 后重试，剩余 ${retryTime - 1} 次: ${errorText}`)
        await new Promise(resolve => setTimeout(resolve, wait))
        return this.sendMessage(text, opt, retryTime - 1, errorOut)
      }
      ...
      try{
        ...
        logger.error('[Gemini] 错误提示图发送失败:', err)
      }
      // 返回空文本，让上层不再追加发送
      return {
        text: '',
